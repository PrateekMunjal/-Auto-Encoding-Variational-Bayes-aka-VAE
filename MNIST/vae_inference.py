    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epoch):
            ...
            if(epoch % 5) == 0:
                ...
                plt.savefig('op/original_new_vae-'+str(epoch)+'.png');
                plt.close();

                print("Reconstructed Images");
                plt.figure(figsize=(n, n));
                plt.imshow(reconstructed, origin="upper", cmap="gray");
                plt.savefig('op/reconstructed_new_vae-'+str(epoch)+'.png');
                plt.close();
        print('Optimization Done !!');
        
def generate(model_num=95):
    with tf.Session() as sess: 
        sess.run(tf.global_variables_initializer());
        ...
        n2 = 10;
        generated = np.empty((28*n2,28*n2));
        for i in range(n2):    
            sample = tf.random_normal([n2,z_dim]);
            generation = sess.run(test_reconstruction,feed_dict={z_sample_test:sample.eval()});
            for j in range(n2):
                        generated[i * 28:(i + 1) * 28, j * 28:(j + 1) * 28] = generation[j].reshape([28, 28]);    
        ...
        #     plt.title('Original (Left) and Reconstructed (Right)');
